...
def extract_navigation_features(edge_features: Dict, ship_features: Dict, include_extended: bool = False) -> np.ndarray:
    """提取导航特征向量（公共函数）
    
    Args:
        edge_features: 边特征字典
        ship_features: 船舶特征字典
        include_extended: 是否包含扩展特征（height_margin 和 draft_ratio）
    ...
    if include_extended:
        ...
        features.append(draft_margin / max(ship_features.get('draft', 5), 0.1))
...
class RiskPredictionModel:
    ...
    def _extract_features(self, edge_features: Dict, ship_features: Dict) -> np.ndarray:
        """提取特征向量"""
        return extract_navigation_features(edge_features, ship_features, include_extended=False)
    ...
    def train(self, X: np.ndarray, y: np.ndarray):
        ...
        # 记录训练集风险范围用于 Min-Max 归一化
        self._risk_min = float(y_train.min())
        self._risk_max = float(y_train.max())
        logger.info("风险评分范围: [%.2f, %.2f]", self._risk_min, self._risk_max)

        train_score = self.model.score(X_train_scaled, y_train)
        val_score = self.model.score(X_val_scaled, y_val)
        logger.info("风险模型 - 训练R2: %.4f, 验证R2: %.4f, 迭代次数: %d",
                     train_score, val_score, self.model.n_estimators_)

        if val_score < train_score - 0.15:
            logger.warning("风险模型可能存在过拟合 (train-R2=%.4f, val-R2=%.4f)",
                           train_score, val_score)

        self.save()
    
    def predict(self, edge_features: Dict, ship_features: Dict) -> float:
        """预测风险评分（Min-Max 归一化至 [0, 100]）"""
        ...
        # Min-Max 归一化到 [0, 100] 区间，提升区分度
        risk_min = getattr(self, '_risk_min', 0)
        risk_max = getattr(self, '_risk_max', 100)
        if risk_max > risk_min:
            risk = (risk - risk_min) / (risk_max - risk_min) * 100
        else:
            risk = 50.0
    ...
    def load(self) -> bool:
        ...
        if os.path.exists(self.model_path):
            ...
            self._risk_min = data.get('risk_min', 0)
            self._risk_max = data.get('risk_max', 100)
...
class PassabilityProbabilityModel:
    ...
    def train(self, X: np.ndarray, y: np.ndarray):
        ...
        logger.info("可达性模型 - 训练准确率: %.4f, 验证准确率: %.4f, F1: %.4f, 迭代次数: %d",
                     train_acc, val_acc, val_f1, self.model.n_estimators_)

        if val_acc < train_acc - 0.1:
            logger.warning("可达性模型可能存在过拟合 (train-acc=%.4f, val-acc=%.4f)",
                           train_acc, val_acc)
...
class MultiTaskNavigationModel:
    ...
    def _extract_features(self, edge_features: Dict, ship_features: Dict) -> np.ndarray:
        """提取特征向量（包含扩展特征）"""
        return extract_navigation_features(edge_features, ship_features, include_extended=True)
