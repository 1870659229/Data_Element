...
4. 自适应阈值：基于统计分布动态确定特征点判断阈值（文献[慕志颖2026]）
"""
...
class NodeExtractor:
    ...
    def _extract_all_in_one(self, df: pd.DataFrame) -> Tuple[List[Dict], List[Dict]]:
        ...
        for idx, (ship_name, group) in enumerate(grouped):
            ...
            # 自适应DP容差：直线型轨迹用更小的tolerance保留更多点
            bearings_temp = np.zeros(n - 1)
            for i in range(n - 1):
                bearings_temp[i] = calculate_bearing(lats[i], lons[i], lats[i+1], lons[i+1])
            if len(bearings_temp) > 1:
                avg_turn = np.mean([abs(calculate_angle_difference(bearings_temp[i], bearings_temp[i+1]))
                                    for i in range(len(bearings_temp)-1)])
            else:
                avg_turn = 0
            
            if avg_turn < 10:  # 直线航道（开阔海域）
                adaptive_dp = max(dp_tolerance * 0.5, 10)
            elif avg_turn > 30:  # 弯曲航道
                adaptive_dp = dp_tolerance
            else:
                adaptive_dp = dp_tolerance * 0.75

            if len(points) > 200:
                simplified_indices = self._sliding_window_dp(points, 200, adaptive_dp)
            else:
                simplified_indices = self._douglas_peucker_indices(points, adaptive_dp)
            ...
            multiplier = self.config.get('adaptive_threshold_multiplier', 1.5)

            if use_adaptive:
                curvatures_all = self._compute_curvatures(lats, lons)
                heading_changes_all = self._compute_heading_changes(bearings)
                adaptive_curv_threshold = self._adaptive_threshold(curvatures_all, multiplier)
                adaptive_heading_threshold = self._adaptive_threshold(heading_changes_all, multiplier)
            # ----------------------------------------
            ...
            for i in simplified_set:
                ...
                if 2 <= i < n - 2:
                    ...
                    if use_adaptive:
                        is_turn = curvature > adaptive_curv_threshold or angle_diff > adaptive_heading_threshold
                    else:
    ...
    def _adaptive_threshold(self, values: np.ndarray, base_multiplier: float = 1.5) -> float:
        """自适应阈值计算（文献[慕志颖2026]）
        基于数据统计分布动态确定阈值:
        - 计算变化率序列的均值和标准差
        - 阈值 = mean + base_multiplier * std
        - 保证阈值至少不低于最小阈值

        Args:
            values: 变化率序列（如COG变化率、SOG变化率）
            base_multiplier: 标准差倍数
        ...
        return max(threshold, 0.0001)  # 曲率最小阈值: 0.0001

    def _compute_curvatures(self, lats: np.ndarray, lons: np.ndarray) -> np.ndarray:
        """向量化计算连续点的曲率（5点法）"""
        n = len(lats)
        if n < 5:
            return np.zeros(n)
        ...
        for i in range(2, n - 2):
            ...
            dist1 = np.sqrt(dx1**2 + dy1**2)
            dist2 = np.sqrt(dx2**2 + dy2**2)
            dist3 = np.sqrt((x[i+1] - x[i-1])**2 + (y[i+1] - y[i-1])**2)

            if dist1 > 1e-6 and dist2 > 1e-6 and dist3 > 1e-6:
                area = 0.5 * cross
                curvatures[i] = 4 * area / (dist1 * dist2 * dist3)

        return curvatures

    def _compute_heading_changes(self, bearings: np.ndarray) -> np.ndarray:
        """向量化计算连续点的航向变化"""
        n = len(bearings)
        if n < 2:
            return np.zeros(n)
