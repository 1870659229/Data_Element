...
class KalmanFilter2D:
    """二维卡尔曼滤波器
    状态向量: [lat, lon, v_lat, v_lon] (4维)
    观测向量: [lat, lon] (2维)
    匀速运动模型
    """

    def __init__(self, process_noise_pos: float = 0.01,
    ...
    def reset(self, lat: float, lon: float, v_lat: float = 0.0, v_lon: float = 0.0):
        self.x = np.array([lat, lon, v_lat, v_lon])
        self.P = np.eye(4) * 1.0

    def update(self, lat: float, lon: float):
        """单步更新，返回 (filtered_lat, filtered_lon)"""
        z = np.array([lat, lon])
        if self.x is None:
            self.reset(z[0], z[1], 0, 0)
    ...
    def smooth_trajectory(self, points):
        """对轨迹点序列进行正向滤波，返回 (lats, lons) 列表"""
        if not points:
            return [], []
        self.reset(points[0][0], points[0][1], 0, 0)
        filtered = []
        for lat, lon in points:
            flat, flon = self.update(lat, lon)
            filtered.append((flat, flon))
        lats = [f[0] for f in filtered]
        lons = [f[1] for f in filtered]
        return lats, lons

    def filter_with_states(self, points):
        """正向滤波，返回所有中间状态供 RTS 平滑器使用
        返回: [(filtered_lat, filtered_lon, x_k, P_k), ...]
        """
        if not points:
            return []
        self.reset(points[0][0], points[0][1], 0, 0)
        ...
        for lat, lon in points:
            flat, flon = self.update(lat, lon)
            states.append((flat, flon, self.x.copy(), self.P.copy()))
        return states


class RtsSmoother:
    """Rauch–Tung–Striebel (RTS) 平滑器
    对前向卡尔曼滤波结果进行反向平滑，得到更优估计
    """

    def __init__(self, process_noise_pos: float = 0.01, process_noise_vel: float = 0.001):
        self.F = np.array([
            [1, 0, 1, 0],
            [0, 1, 0, 1],
            [0, 0, 1, 0],
    ...
    def smooth(self, forward_states):
        """反向平滑
        forward_states: [(filtered_lat, filtered_lon, x_k, P_k), ...]
        返回: [(smoothed_lat, smoothed_lon), ...]
        """
        ...
        smoothed = [None] * n
        smoothed[n - 1] = (forward_states[n - 1][0], forward_states[n - 1][1])
        x_s = forward_states[n - 1][2].copy()
        P_s = forward_states[n - 1][3].copy()

        for k in range(n - 2, -1, -1):
            x_k = forward_states[k][2]
            P_k = forward_states[k][3]

            # 预测
            x_pred = self.F @ x_k
            P_pred = self.F @ P_k @ self.F.T + self.Q

            # 平滑增益
            G = P_k @ self.F.T @ np.linalg.inv(P_pred)
            x_s = x_k + G @ (x_s - x_pred)
            P_s = P_k + G @ (P_s - P_pred) @ G.T

            smoothed[k] = (float(x_s[0]), float(x_s[1]))
...
class DataPreprocessor:
    ...
    def _detect_drift_multidim(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if all_features:
            try:
                ...
                clf = IsolationForest(
                    contamination=self.cleaning_config.get('iforest_contamination', 0.01),
                    random_state=42,
                    n_estimators=self.cleaning_config.get('iforest_n_estimators', 100))
    ...
    def _kalman_smooth(self, group: pd.DataFrame) -> pd.DataFrame:
        """2D卡尔曼滤波 + RTS平滑器（双向平滑）"""
        q_pos = self.smoothing_config.get('kf_process_noise_pos', 0.01)
        q_vel = self.smoothing_config.get('kf_process_noise_vel', 0.001)
        r = self.smoothing_config.get('kf_measurement_noise', 0.1)
        use_rts = self.smoothing_config.get('use_rts_smoother', True)

        lats = group['纬度'].values
        lons = group['经度'].values
        points = list(zip(lats, lons))

        kf = KalmanFilter2D(process_noise_pos=q_pos, process_noise_vel=q_vel,
                            measurement_noise=r)
        forward_states = kf.filter_with_states(points)

        if use_rts:
            smoother = RtsSmoother(process_noise_pos=q_pos, process_noise_vel=q_vel)
            smoothed = smoother.smooth(forward_states)
            group['纬度'] = [s[0] for s in smoothed]
            group['经度'] = [s[1] for s in smoothed]
        else:
            group['纬度'] = [s[0] for s in forward_states]
            group['经度'] = [s[1] for s in forward_states]
        return group
